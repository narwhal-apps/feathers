...
fn log_returns_commits_in_reverse_chronological_order() {
    ...
    let page = commit::log(&r, LogOpts { max: 10, ..Default::default() }).unwrap();
...
fn log_pagination_emits_next_cursor() {
    let pairs: Vec<(String, String)> = (1..=10)
        ...
        .collect();
    let pair_refs: Vec<(&str, &str)> = pairs.iter().map(|(p,c)| (p.as_str(), c.as_str())).collect();
    let dir = common::fixtures::seeded_repo(&pair_refs);
    let r = repo::open(dir.path()).unwrap();
    let page1 = commit::log(&r, LogOpts { max: 5, ..Default::default() }).unwrap();
    assert_eq!(page1.commits.len(), 5);
    assert!(page1.next_cursor.is_some());
    let page2 = commit::log(&r, LogOpts {
        max: 5,
    ...
    assert!(page2.commits.iter().all(|c| !page1.commits.iter().any(|p| p.oid == c.oid)));
}
